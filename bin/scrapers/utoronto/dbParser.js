/*

	{
		...
		C : [{
			...
			instructors:[String]	//The teacher name
			times:[{
...
function pushData(line){
	...
	line.meeting_sections.map(function(a){
		delete a.breadths;
		
		a.times	=	a.times.map(function(t){
			return {
				days:[["MONDAY","TUESDAY","WEDNESDAY","THURSDAY","FRIDAY","SATURDAY","SUNDAY"].indexOf(t.day)],
				start:t.start*60,
				end:t.end*60,
				location:t.location
			}
		});
		switch(a.code.substr(0,1)){
			case 'L':
				sections.L.push(a);
			break;
			case 'P':
	...
	line.meeting_sections	=	sections;
}
...
Parser	=	function(termCode,year){
	return {
		pushData	:	function(main,times){
			...
			var UFV		=	mongo.get('utorontoCourse');
		...
		createInstant:function(){
			...
			UFV.find({},function(e,docs){
				docs.forEach(function(doc,i,a){
					...
					db.query('INSERT INTO course.utoronto_course_instant (tags,name,code,term,year) VALUES (?,?,?,?,?)',[tags,doc.name,doc.title,doc.term,doc.year]);
				});
			});

			//ALTER TABLE utoronto_course_instant add fulltext INDEX utorontoCourse (tags)
		},
		preMangle:function(){
			var UFV		=	mongo.get('utorontoCourse');
			UFV.find({},function(e,docs){
			...
			_.get({
				...
				json:true,
					...
					e.response.docs.forEach(function(v,i,a){
						db.query('INSERT INTO course.utoronto_teacher_rating (teacherName,rating,votes) VALUES (?,?,?)',[
