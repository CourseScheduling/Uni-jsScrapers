...
function Mangler(course){
	...
	typeArray[0].forEach(function(v,i,a){
		depther([v],typeArray.length,typeArray);
	});
	...
	function	depther(pos,layerIndex,layerArray){
		...
		if(layerIndex<0||layer.length==0)
		...
		var sectionC	=	pos.length;
			while(sectionC--){
				...
				while(timeL--){
					while(timeS--){
						...
						var lST	=	pos[layerC].times[timeL];
			...
			if(layerIndex==0)
				...
				return depther(pos.concat(layer[layerC]),layerIndex--,layerArray);
		}
	}
